...
from order_refunds import refund_order_wallets
    ...
    for oid, idxs in grouped.items():
        try:
            if new_status == "refunded":
                count = refund_order_wallets(
                    orders_collection=orders_collection, order_id=oid, indices=idxs,
                    actor_admin_id=actor_admin_id, reason=reason, api_status=api_status,
                )
                updated_lines += count
                if count:
...
def _apply_status_change(order_ids: List[ObjectId], new_status: str, reason: str = "manual", actor_admin_id=None, orders_collection=orders_col, source: str = "main") -> Tuple[int, List[str]]:
    ...
    for oid in order_ids:
        try:
            if new_status == "refunded":
                count = refund_order_wallets(
                    orders_collection=orders_collection, order_id=oid,
